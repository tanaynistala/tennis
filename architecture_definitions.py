...
class FrameModel(nn.Module):
    def __init__(self, backbone, num_classes=-1, swap=False, **kwargs):
        ...
        with self.name_scope():
            ...
            self.classes = None
            if num_classes > 0:
                self.classes = nn.Linear(num_classes, flatten=True)
    ...
    def __init__(self, model, num_classes=-1, hidden_size=128, type='gru', **kwargs):
        ...
        with self.name_scope():
            if model is not None:
                self.td = TimeDistributed(model.backbone)
            if type == 'lstm':
                self.rnn = nn.LSTM(hidden_size, layout="NTC", bidirectional=True)
            else:
                self.rnn = nn.GRU(hidden_size, layout="NTC", bidirectional=True)
            self.classes = None
            if num_classes == 0:
                self.classes = model.classes
            elif num_classes > 0:
